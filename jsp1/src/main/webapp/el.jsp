		...
		<%-- EL
         ...
         <%-- 상태정보유지:
          ...
          EL에서 상태 변수를 찾는 순서 : ${ var }
            ${ pageScope.var } ==> $ { requestScope.var } => $ { sessionScope.var }
            => $ { applicationScope }
          ...
          <p>
            var1 : <%= pageContext.getAttribute("var1") %>
            <br />
            var2 : <%= request.getAttribute("var2") %>
            <br />
            var2 : <%= session.getAttribute("var3") %>
          </p>
         
         <h2>EL을 사용한 상태정보 읽기</h2>
         <p>
