<%@ page import="java.time.LocalDateTime"%> <%-- 자바 클래스의 import 문장 --%>
...
<html>
	...
	<body>
        ...
        => 웹 서버에서 실행되는 요청을 처리하는 작은 자바 프로그램
        => Java 클래스로 작성. HttpServlet 클래스를 상속.
        => 서블릿 객체의 생성과 서블릿 메서드 호출은 WAS가 담당.
         * 최초 요청 : 서블릿 객체 생성. -> doGet, doPost 메서드 호출 -> 클라이언트로 응답 전달.
        ...
        3. JSP 구성 요소(태그)
            ...
            (4) 스크립트릿(Scriptlet) : <% ... %>
                JSP가 Java로 변환이 될 때, _jspService() 메서드 안에 포함되는 자바 코드.
                지역변수 선언, 조건문, 반복문, 메서드 호출...
            (5) 식, 표현식(expression) : <%= ... %>
                JSP가 자바로 변환될 때, out.print() 메서드의 argument로 전달되는 값
                HTML에 바로 삽입되는 "값"
         --%>
